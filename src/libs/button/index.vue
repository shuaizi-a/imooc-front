<template>
  <button
    class="text-sm text-center rounded duration-150 flex justify-center items-center overflow-hidden"
    :class="[typeEnum[type], sizeEnum[sizeKey].button, { 'active:scale-105': isActiveAnim }]"
    @click.stop="onBtnClick"
  >
    <!-- 展示 loading -->
    <div v-if="loading" class="absolute top-0 left-0 bottom-0 right-0 flex justify-center items-center bg-zinc-100/60">
      <m-svg-icon name="loading" class="w-2 h-2 animate-spin"></m-svg-icon>
    </div>
    <!-- icon 按钮 -->
    <m-svg-icon v-if="icon" :name="icon" class="m-auto" :class="sizeEnum[sizeKey].icon" :color="iconColor" :fillClass="iconClass"></m-svg-icon>
    <!-- 文字按钮 -->
    <slot v-else />
  </button>
</template>
