    ...
    >
      ...
      <div class="h-full w-full px-1 pb-4 text-sm mt-2 xl:w-2/3 xl:pb-0">
        ...
        <div class="py-1 xl:absolute xl:right-[16%] xl:text-center">
          ...
          <!-- 头像部分 -->
          <div @click="onAvatarClick" class="relative w-[80px] h-[80px] group xl:cursor-pointer xl:left-[50%] xl:translate-x-[-50%]">
            <img v-lazy :src="$store.getters.userInfo.avatar" alt="" class="rounded-[50%] w-full h-full xl:inline-block" />
        ...
        <!-- 保存修改 -->
        <m-button class="w-full mt-2 mb-4 dark:text-zinc-300 dark:bg-zinc-800 xl:w-[160px] xl:ml-[50%] xl:translate-x-[-50%]" @click="onChangeProfile"
          >保存修改</m-button
        >
        <!-- 移动端退出登录 -->
        <m-button
