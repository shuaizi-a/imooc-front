  ...
  <m-popover class="flex items-center" placement="bottom-left">
    <template #reference>
      ...
      >
        ...
        <!-- vip 标记 -->
        <m-svg-icon v-if="$store.getters.userInfo.vipLevel" name="vip" class="w-1.5 h-1.5 absolute right-[16px] bottom-0"></m-svg-icon>
      </div>
      <div v-else>
    ...
    <div class="w-[140px] overflow-hidden" v-if="$store.getters.token">
      ...
      >
        <m-svg-icon :name="item.icon" class="w-1.5 h-1.5 mr-1" fillClass="fill-zinc-900 dark:text-zinc-300"></m-svg-icon>
